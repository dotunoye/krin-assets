document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.pop-in, .jump-text').forEach(el => revealObserver.observe(el));
  ...
  document.querySelectorAll('.tabs').forEach(tabList => {
    tabList.addEventListener('click', event => {
      ...
      scope.querySelectorAll('.tab-panel').forEach(panel => { panel.hidden = panel.id !== button.dataset.tab; });
    });
  });
  ...
  document.querySelectorAll('form[data-demo-form]').forEach(form => {
    form.addEventListener('submit', event => {
      ...
      if (status) {
        status.textContent = 'Thank you! Your message is ready for our team. We’ll be in touch shortly.';
